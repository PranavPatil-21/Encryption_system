...
(
    ...
    total_samples,
    theta1_initial,
    angularVelocity_initial_1,
    theta2_intial,
    angular_velocity_initial_2,
    mass1,
    mass2,
    length_1,
    length_2,
    gravity,
) = Support.readKey(keyFile)
x1, x2, y1, y2 = DoublePendulum.getCoordinates(
    total_time,
    int(total_samples),
    theta1_initial,
    angularVelocity_initial_1,
...
try:
    ...
    if data_type == 1:  # image
        ...
        responseBody["filepath"] = str(decryptedBytes[1:], "utf-8")
        responseBody["success"] = True
    else:
        responseBody["filepath"] = "Something Invalid Happened"
        responseBody["success"] = False
except Exception as eobj:
    print(eobj)
    responseBody["filepath"] = f"Exeption ocurred with message {eobj}"
    responseBody["success"] = False
